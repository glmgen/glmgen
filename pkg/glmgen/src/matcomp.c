...
void builddmat(int n, int k, double *x, double *dmat){
  int i, j, row, col;
  double *tmp = (double*)malloc(sizeof(double)*(n*(n-1)));
  for (i=0; i<n-1; i++) {
      tmp[i*n+i] = -1;
      tmp[i*n+i+1] = 1;
    }
...
void l2d(int n, int k, double *a, double *x) {
  double *drow = (double*)malloc((k+2)*sizeof(double));
  builddrow(k+1,drow);

  // Compute D*a
  d(n,k+1,drow,a,x+k+1);
...
void solveddt(int n, int k, double *b, double *a, double *c, double *l,
  ...
  for (int i=k+1; i<n; i++) a[i] = b[i-k-1];
  ...
  double *wrk = (double*)malloc(n*(k+1)*sizeof(double));
  char trans = 'n';
  int nr=n, nc=k+1, nrhs=1, nw=n*(k+1), info=0;
